...
def plot(year,month):
    ...
    for _year in range(2009,year):
        for _month in range(1,13):
            data_90.append(CalculateResult.objects.get(city_or_area=False,area=CityArea.QUANGUO_90,year=_year,month=_month))
    for _month in range(1,month+1):
        data_90.append(CalculateResult.objects.get(city_or_area=False,area=CityArea.QUANGUO_90,year=year,month=_month))
    ...
    p.plot_year_on_year_radio_90(data=year_on_year_index_90,saveurl=('media/image/'+str(year)+'_'+str(month)+'yearonyearplot_90.png'))
    # 绘制全国环比图
    chain_index_90 = []
    for i in range(1,lenth_90):
        chain_index_90.append(data_90[i].chain_index)
    p.plot_chain_radio_90(data=chain_index_90,saveurl=('media/image/'+str(year)+'_'+str(month)+'chainplot_90.png'))
    # 绘制指数图
    index_90 = []
    ...
    p.plot_index_by_area_90(index_area_90,saveurl=('media/image/'+str(year)+'_'+str(month)+'index_by_buildarea_90.png'))
    p.plot_vol_index_by_area_complex_90(vol_area_90,index_area_90,'media/image/'+str(year)+'_'+str(month)+'index_by_buildarea_c_90.png')
    ...
    p.plot_index_by_block_90(index_block_90,saveurl=('media/image/'+str(year)+'_'+str(month)+'index_by_block_90.png'))
    p.plot_vol_index_by_block_complex_90(vol_block_90,index_block_90,'media/image/'+str(year)+'_'+str(month)+'index_by_block_c_90.png')
    ...
    p.plot_vol_index_by_line_complex(vol_block_90,index_block_90,'media/image/'+str(year)+'_'+str(month)+'index_by_line_c.png')
    ...
    for i in range(0,3):
        ...
        for j in range(0,lenth):
            index_block_90[i].append(data[j].index_value_base09)
            vol_block_90[i].append(data[j].trade_volume)
    p.plot_index_by_block_90(index_block_90,saveurl=('media/image/'+str(year)+'_'+str(month)+'index_by_s_area.png'))
    p.plot_vol_index_by_block_complex_90(vol_block_90,index_block_90,'media/image/'+str(year)+'_'+str(month)+'index_by_s_area_c.png')

    # 绘制地区 -简版与完整版 90
    index_block_90 = [[] for i in range(0,7)]
    vol_block_90 = [[] for i in range(0,7)]
    block_90 = [CityArea.DONGBEI,CityArea.HUABEI,CityArea.HUADONG,CityArea.HUAZHONG,CityArea.HUANAN,CityArea.XINAN,CityArea.XIBEI]
    for i in range(0,7):
        data = []
        for _year in range(2009,year):
            for _month in range(1,13):
        ...
        for j in range(0,lenth):
            index_block_90[i].append(data[j].index_value_base09)
            vol_block_90[i].append(data[j].trade_volume)
    p.plot_index_by_7area(index_block_90,saveurl=('media/image/'+str(year)+'_'+str(month)+'index_by_7area.png'))
    p.plot_vol_index_by_7area_complex(vol_block_90,index_block_90,'media/image/'+str(year)+'_'+str(month)+'index_by_7area_c.png')
    ...
    for city in city_list_90:
        ...
        for i in range(0,lenth):
            if data[i].index_value==0:
                ...
                last_value = i
            city_vol.append(data[i].trade_volume)
        p.plot_vol_index_90(volume=city_vol,index=city_index,saveurl=('media/image/'+str(year)+'_'+str(month)+'volindex_'+city.code+'_90.png'))
    return True