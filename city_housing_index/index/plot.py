...
class PlotReport:
    ...
    def plot_vol_index_by_block_complex(self, volume, index, saveurl):
        ...
        ax.bar(np.arange(len(volume[1])) + 0.20, volume[1], 0.20, color='white', ec='gray', lw=1, hatch='/',
               label='中部交易量')
        ax.bar(np.arange(len(volume[2])) + 0.40, volume[2], 0.20, color='white', ec='gray', lw=1, hatch='.',
               label='西部交易量')
        ax.spines['left'].set_position(('data', -1))
        ax.legend(loc=1)
        ax.set_xlim(xmin=-1)
    ...
    def plot_vol_index_by_line_complex(self, volume, index, saveurl):
        ...
        ax.bar(np.arange(len(volume[1])) + 0.15, volume[1], 0.15, color='white', ec='gray', lw=1, hatch='/',
               label='二线城市交易量')
        ax.bar(np.arange(len(volume[2])) + 0.30, volume[2], 0.15, color='white', ec='gray', lw=1, hatch='.',
               label='三线城市交易量')
        ax.bar(np.arange(len(volume[3])) + 0.45, volume[3], 0.15, color='white', ec='gray', lw=1, hatch='*',
               label='四线城市交易量')
        ax.spines['left'].set_position(('data', -1))
        ax.legend(loc=1)
        ax.set_xlim(xmin=-1)
    ...
    def plot_vol_index_by_7area_complex(self, volume, index, saveurl):
        ...
        ax2.set_yticks(range(100, int(max(
            [max(index[0]), max(index[1]), max(index[2]), max(index[3]), max(index[4]), max(index[5]),
             max(index[6])]) + 150), 50))
        ax2.set_ylabel('指数', fontproperties='SimHei')
        ax2.legend(loc=2)
        plt.savefig(saveurl, dpi=200)
    ...
    def plot_index_by_7area(self, data, saveurl):
        ...
        plt.ylim(ymin=100)
        plt.xticks(range(0, len(data[0]), 4))
        plt.yticks(range(100, int(max(
            [max(data[0]), max(data[1]), max(data[2]), max(data[3]), max(data[4]), max(data[5]), max(data[6])]) + 150),
                         50))
        plt.grid(axis="y", linestyle='--', color='gray')
        plt.xticks(rotation=90, fontsize=10)
        plt.yticks(fontsize=10)
