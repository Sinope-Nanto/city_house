...
def update_user_sms_code(mobile) -> str:
    ...
    user_sms = UserSMSCode.objects.filter(user_id=user_profile.user_id).first()
    ...
    if user_sms:
        ...
        user_sms = UserSMSCode(user_id=user_profile.user_id, code=verify_code, expire=expire)
    user_sms.save()
    return verify_code
...
def create_token(mobile):
    ...
    user_session, created = UserSession.objects.get_or_create(user_id=user_profile.user_id)
...
def register(**kwargs) -> (bool, str):
    ...
    try:
        user_profile = UserProfile(user_id=user_id, name=name, mobile=mobile, city_id=city, identity=identity, identity_image=image_file)
...
def decode_image(encoded_image, file_name):
    decoded_bytes = base64.decodebytes(encoded_image)
